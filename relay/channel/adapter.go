...
type TaskBillingField = relaycommon.TaskBillingField

type TaskBillingCapability = relaycommon.TaskBillingCapability

// TaskBillingCapabilityProvider is implemented by task adaptors that can
// produce a stable canonical billing input for the mapped upstream model.
// Returning nil means that the current model has no canonical schema.
type TaskBillingCapabilityProvider interface {
