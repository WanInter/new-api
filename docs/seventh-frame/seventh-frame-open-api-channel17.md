# 第七帧开放 API 文档 - 限时福利（通道17）

文档生成时间：2026-07-21T06:51:03.587Z

本文档仅适用于当前已上架的 限时福利（通道17）（channel17）。如果后台下架通道或模型，请重新下载最新文档。
...
| --- | --- | --- | --- | --- | --- |
seedance-2.0--person-stripe--6e9f7f9c--voice-tone--a7f8bf20 | 七帧sd2.0 | 4s, 5s, 6s, 7s, 8s, 9s, 10s, 11s, 12s, 13s, 14s, 15s | 720p | 图片 9 / 音频 3 / 视频 0 | 36/条
...
响应会返回所有已上架模型。接入 限时福利（通道17） 时，请筛选 `channel === "channel17"` 的模型。
...
curl -X POST "https://diqizhen.jytt4.cn/api/v1/video/generations" \
  ...
  -d '{
    "channel": "channel17",
    "model": "seedance-2.0--person-stripe--6e9f7f9c--voice-tone--a7f8bf20",
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| channel | string | 是 | 固定为 `channel17` |
...
```json
{
  "generation": {
    ...
    "id": "task_xxx",
    "status": "queued",
    "processingStage": "preparing_assets",
    "progress": 0,
    "channel": "channel17",
    "model": "seedance-2.0--person-stripe--6e9f7f9c--voice-tone--a7f8bf20",
    "billing": {
...
```bash
curl -X GET "https://diqizhen.jytt4.cn/api/v1/usage?channel=channel17" \
  -H "Authorization: Bearer <API_KEY>"
```
...
```json
{
  "error": {
    ...
    "message": "channel 必须是 channel1 到 channel17 之一。"
  }
}
```
