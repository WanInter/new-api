...
| 对外模型 | `seedance-2-720p-933` |
| 上游模型 | `seedance-2-720p` |
| 密钥 | 以 `hm_` 开头的 API Key |
| 鉴权 | `Authorization: Bearer <API_KEY>` |
| 网关转发提交 | `POST /v1/video/generations/multipart` |
| 任务查询 | `GET /v1/video/generations/{video_id}` |

在渠道模型列表中添加公开模型名 `seedance-2-720p-933`。适配器会自动将其
转换为 Axmgc 上游模型 `seedance-2-720p`；若配置了显式模型映射，则以映射值为准。
...
| --- | --- | --- | --- | --- |
| `seedance-2-720p` | 720p | 固定 15 秒 | 9 图片 / 3 视频 / 3 音频 | ¥4.50/次 |
