...
// WaffoPancakeTopUpProviderPayload stores provider-side identifiers returned
// when creating a wallet top-up checkout session. OrderMerchantExternalID is
// still the authoritative webhook lookup key; these fields are retained for
// audit/debug compatibility with existing orders.
